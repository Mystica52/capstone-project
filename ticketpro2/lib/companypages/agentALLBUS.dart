...
import 'package:ticketpro2/companypages/agentBusDetails.dart';
...
class AgentAllBuses extends StatefulWidget {
  final token ;
  
  const AgentAllBuses({@required this.token, super.key});

  @override
  _AgentAllBusesState createState() => _AgentAllBusesState();
}

class _AgentAllBusesState extends State<AgentAllBuses> {
        ...
        child: isIssuedBy
            ? ElevatedButton(
                onPressed: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => AgentBusDetails(token: widget.token, origin: origin, destination:destination,)),
                  );
                },
